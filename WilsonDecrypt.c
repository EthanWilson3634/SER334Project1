/**
* (This program decrypts an array of strings using pointer arithmetic)
*
...
void sort_chunks() {
    ...
    for(int i = 0; i < NUMBER_OF_CHUNKS - 1; i++){
        ...
        for(int j = i + 0; j < NUMBER_OF_CHUNKS; j++){
            char* ptr = chunks[j];
            if(*ptr < *min) {
                min = ptr;
            }
...
void decrypt_chunks() {
    ...
    for(int i = 0; i < NUMBER_OF_CHUNKS; i++){
        char* innerPtr = chunks[i];
        for(int j = 0; j < CHUNK_LENGTH - 1; j++){
            *innerPtr -= DECRYPTION_SHIFT;
            innerPtr++;
        }
    }
}
